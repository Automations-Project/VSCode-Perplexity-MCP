...
- **Five supported browser runtimes.** [packages/extension/src/browser/browser-detect.ts](packages/extension/src/browser/browser-detect.ts) probes Google Chrome → Microsoft Edge → system Chromium → Brave → patchright's bundled Chromium. The first four launch via Patchright's `channel`; the fifth comes from `patchright install chromium` into VS Code's globalStorage, managed by [BrowserDownloadManager](packages/extension/src/browser/browser-download.ts). Selection flows through the env vars `PERPLEXITY_BROWSER_CHANNEL` (`chrome` / `msedge` / `chromium`) and `PERPLEXITY_BROWSER_PATH`, both evaluated by [packages/mcp-server/src/config.ts](packages/mcp-server/src/config.ts). The extension's [AuthManager](packages/extension/src/mcp/auth-manager.ts) syncs these onto `process.env` so the detached MCP daemon inherits the active selection.
