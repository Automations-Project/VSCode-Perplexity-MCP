...
### Fixed — external MCP clients bricked the shared daemon after an in-place update (version gate over-reach)

- **An external stdio client (Grok, Cursor, Claude Desktop, Cline, …) killed the running daemon and could not rebuild it** ([`attach.ts`](packages/mcp-server/src/daemon/attach.ts), [`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts)). The code-graph version gate (added for the reap-stale-daemon work) was applied on the **attach** path: an updated `start.mjs` running mcp 0.8.59 would find a still-running 0.8.58 daemon, **reclaim (kill) it**, then try to spawn a 0.8.59 replacement via the default `spawnDetachedDaemon` — which in the extension-bundled layout looks for `cli.mjs`/`cli.js` siblings that don't exist (there the daemon entry is `server.mjs daemon start`). The spawn failed silently and the client got `handshake failed / Timed out waiting for daemon startup`, with `daemon.lock` gone. Reported live from Grok Build after an in-place extension update; confirmed from the diagnostics bundle (`daemon.lock.MISSING`, stderr `reclaiming daemon pid … lock claims code graph 0.8.58, need 0.8.59`).
  - **Fix (policy):** an external client is a **guest** of the daemon, not its owner — it now attaches to whatever healthy daemon is running and never evicts it. Killing the shared daemon disrupts every *other* attached client and creates a two-owner port-coordination mess, and the narrow risk of a mildly-stale daemon (a code-split import could fail) is far better than a daemon dead for everyone. Version reconciliation stays the **extension host's** job: it reaps a stale-version daemon on activation (now tree-killing its browser) and gates its own `ensureBundledDaemon` with a *working* spawner, so a normal editor reload after an update brings up a matching daemon. `expectedMcpVersion` is gone from the attach path.
  - **Fix (robustness):** `resolveCliEntry` now falls back to the module's **own** file when no `cli.*` sibling exists — in the bundle that's `server.mjs`, itself a valid `daemon start` entry. Without this, an external client that finds *no* daemon at all could not start the shared one and silently fell back to its own in-process browser, defeating the N-clients-one-Chromium design.
- Verification: [`test/daemon/resolve-cli-entry.test.js`](packages/mcp-server/test/daemon/resolve-cli-entry.test.js) (resolves to a real on-disk entry; a regression lock asserting `attach.ts` never passes `expectedMcpVersion`); the existing attach round-trip suite stays green.

### Added — xAI Grok (Grok Build) auto-config

- **Grok is now a first-class auto-config target** ([`constants.ts`](packages/shared/src/constants.ts), [`auto-config/index.ts`](packages/extension/src/auto-config/index.ts)). It reads `~/.grok/config.toml` with the same `[mcp_servers.<name>]` TOML shape as Codex CLI (command/args/env + `enabled = true`), and `AGENTS.md` for project rules — so it slots into the existing TOML writer with no new machinery: user-scoped config path, detection via `~/.grok`, `Perplexity.autoConfigureGrok` setting, the missing-config nudge, and an upsert that preserves the user's `[cli]`/`[ui]`/`[marketplace]` and other `[mcp_servers.*]` blocks. HTTP-loopback stays evidence-gated (stdio is the verified path — the reporter ran it live). Grok mark added to the dashboard IDE cards using the official lobehub Grok artwork, vendored as an SVG to keep `@lobehub/icons`' `antd` + `@lobehub/ui` peer deps out of the lean webview bundle.
- Verification: [`tests/auto-config.grok.test.ts`](packages/extension/tests/auto-config.grok.test.ts) (7) — metadata shape, `~/.grok/config.toml` path resolution, block write with `enabled = true`, preservation of other servers/sections, idempotent re-apply, and detection on/off.
...
- **Daemon kills no longer orphan the browser (Windows)** ([`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts), [`stale-version.ts`](packages/extension/src/daemon/stale-version.ts), new [`process-tree.ts`](packages/mcp-server/src/process-tree.ts)). `process.kill(pid, "SIGTERM")` on Windows is `TerminateProcess` — the daemon's cleanup handler never runs, so its Chrome child survived **every** reclaim (wedge, version-gate, #15 stale-fence, force-stop, and the extension's activation reaper) still holding the profile singleton. All five paths now use `taskkill /PID … /T /F` on Windows (the tunnel providers learned this first); POSIX keeps SIGTERM-then-SIGKILL so the daemon's own handler closes its browser gracefully.
- **Orphan squatters are evicted, not tab-spammed** ([`client.ts`](packages/mcp-server/src/client.ts) `prepareProfileForLaunch`, [`process-tree.ts`](packages/mcp-server/src/process-tree.ts)). On a real ProcessSingleton collision (retry ≥1 — the happy path spawns nothing), the client hunts processes whose command line carries `--user-data-dir=<browser-data>` and tree-kills them before retrying. **Ownership-guarded**: if a *live foreign daemon* holds `daemon.lock`, nothing is killed — we are the trespasser (an in-process stdio fallback, say), and killing the rightful owner's browser would sabotage every attached client (issue #8 single-owner invariant). The Windows process query passes the marker via **environment variable**, not inline — a literal in the `-Command` string sits in PowerShell's own command line and the query would match (and evict) its own shell.
- **Reinit storms coalesce** ([`client.ts`](packages/mcp-server/src/client.ts)). Reinit is idempotent — it reloads whatever is on disk *now* — so N requests arriving while one is in flight collapse into at most **one** follow-up run instead of 138 queued teardown+bootstrap cycles. The follow-up keeps the **newest** passphrase; dropping a fresh one would strand a profile switch. A failing run rejects its joiners but never wedges the next request.

#### Added

- **Idle browser parking** ([`client.ts`](packages/mcp-server/src/client.ts), `PERPLEXITY_BROWSER_IDLE_PARK_MS`, default 30 min, `0` disables). The Chromium tree is the daemon's real cost (~300–600 MB RSS); the daemon itself is a small HTTP server. After the idle window with no page work, the browser is closed and the next tool call resurrects it lazily (`search`/`computeASI` now lazy-init instead of throwing `"Client not initialized"`). Parking is deliberately **not** `shutdown()`: shutdown flips `authenticated=false` and writes an anonymous daemon-status, and a parked daemon is still logged in — the dashboard must not start telling users to re-login because we saved them RAM. Park runs as a scheduler barrier (never yanks the page from an in-flight op), steps aside if work queued up while it waited, and the timer is `unref()`d so short-lived CLI/stdio processes still exit naturally. The trade — the first call after a long idle repays the bootstrap (~10–30 s) — is documented next to the knob.

#### Auto-update & status coverage (verified, no new code)

- Upgrade path end-to-end: the extension's activation reaper stops an old-build daemon (now tree-killing its browser too); the **code-graph version gate** makes `ensureDaemon`/plain `attach` refuse a mismatched daemon and replace it; VS Code re-resolves on the port change; and the one-shot post-upgrade toast (`maybeNotifyUpgradeComplete`, wired at activation) tells legacy users external clients may need one reconnect. Eviction/park/reclaim decisions all log to `daemon.log` with reasons.

#### Verification

- New: [`test/process-tree.test.js`](packages/mcp-server/test/process-tree.test.js) (8) — tree-kill of a real parent+child on Windows (the orphan-creation case itself), self-pid/dead-pid safety, marker matching against **live** processes (a bystander is never matched; the PowerShell query does not match itself), eviction when nobody or a **dead** lock-holder owns the profile, and the **refusal** when a live foreign daemon owns it. Test fixture note: `node -e "script" --user-data-dir=…` dies at spawn (node parses the marker as an unknown node option) — sleepers pass it after `--`, without which the suite passes/fails for the wrong reasons.
- New: [`test/client-lifecycle.test.js`](packages/mcp-server/test/client-lifecycle.test.js) (9) — 41-deep reinit storm → exactly 2 underlying runs; newest passphrase wins; failure rejects joiners without wedging; sequential reinits unaffected; parking closes the browser while **preserving auth state**; new work cancels the park; `=0` disables; a parked client's `search()` provably reaches a live page again; park steps aside under queued work.
- Updated [`stale-version.test.ts`](packages/extension/tests/stale-version.test.ts) (14) for the platform fork: Windows never uses `process.kill` (tree-kill only), POSIX keeps the graceful SIGTERM contract.
- Full suite **1344 passed / 154 files**, typecheck clean across all four packages.

### Fixed — visible browser window: Phase 2 headed-offscreen default reverted to headless (opt-in)

- **Users saw a permanent, fully visible Chrome window** ([`client.ts`](packages/mcp-server/src/client.ts) `resolvePhase2Headless`). The unreleased #12 mitigation made the persistent search context launch headed-offscreen by default on Windows/macOS. The off-screen trick (`--window-position=-32000,-32000`) was designed for the **~20-second** Phase 1 bootstrap (issue #9); Phase 2's window is **long-lived**, has a taskbar presence, and ordinary events — clicking the taskbar icon, display/DPI changes, window-manager "rescue" of off-screen windows — re-place it fully on-screen. Confirmed on a real Windows device within a day of the change. This was an explicitly flagged risk of the mitigation ("re-opens issue #9's blast radius"); it materialized, so the default is **headless again on every platform**. `PERPLEXITY_PERSISTENT_HEADED=1` remains as the explicit opt-in for anyone genuinely CF-blocked on the reuse path (ignored on display-less Linux, where a headed launch would kill the daemon at start).
- **The evidence for the headed default also weakened**: #12's diagnosis was never confirmed for the `cf_clearance` **reuse** path (the repro only showed headless cannot *solve* a challenge, which Phase 1 exists for), the reporter's disambiguation questions remain unanswered, and the new [#15](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/15) revealed a **5-daemon pile-up on that same machine** fighting over one `browser-data` — which produces the identical `"Target page, context or browser has been closed"` error. Reverting an unproven fix that caused a proven regression.

### Fixed — stale-reclaim now fences a live lock holder (issue #15)

- **`getDaemonStatus(reclaimStale: true)` released the lockfile but never terminated the losing process** ([`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts)) — [#15](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/15). "Stale" usually means the pid is dead, but `isStale()` also condemns a **live** holder whose health responder echoes a different uuid than the lock. Dropping the lock while that process lived on left an orphan daemon holding Chromium and the `browser-data` profile singleton; the next ensure then spawned a fresh daemon straight into a profile-lock fight with it — the reporter counted **five** simultaneous daemons, one four days old, every in-flight call dying with `"Target page, context or browser has been closed"` and external stdio clients losing their session permanently. A live, foreign, stale holder is now SIGTERM→SIGKILLed before the lock is released (same fence as the #14 wedge and version-gate reclaims); a dead-pid stale lock is released without signalling, and `reclaimStale: false` remains strictly observational. Combined with the already-landed reinit crash fix, wedge reclaim, and code-graph version gate, this closes the pile-up loop end to end. Remaining #15 suggestions (profile-singleton fence at spawn, parent-death sweep, pinned-port default) are tracked on the issue as follow-ups.

#### Verification

- Updated [`test/client-launch-options.test.js`](packages/mcp-server/test/client-launch-options.test.js): headless default on all three platforms (the visible-browser regression guard), opt-in headed via `PERPLEXITY_PERSISTENT_HEADED=1`, the opt-in refused on display-less Linux, `=0` and `PERPLEXITY_HEADLESS_ONLY` still forcing headless.
- New [`test/daemon/launcher-stale-fence.test.js`](packages/mcp-server/test/daemon/launcher-stale-fence.test.js) (3) — a **real live process** answering health with a mismatched uuid is killed before its lock is released; `reclaimStale: false` stays side-effect free; a dead-pid lock is released without signalling. **Negative-tested:** with the fence disabled, the kill test fails while the two absence-asserting tests still pass.
...
#### Investigation notes (evidence for the pre-fix races)

- **No mutex anywhere.** The daemon builds exactly one `PerplexityClient` ([`launcher.ts:334`](packages/mcp-server/src/daemon/launcher.ts#L334), handed over as `createClient: () => client` at `:518`) and memoizes it in `getClient()` ([`server.ts:170-190`](packages/mcp-server/src/daemon/server.ts#L170-L190)). It holds exactly one `page` ([`client.ts:969`](packages/mcp-server/src/client.ts#L969)), assigned once per init. A repo-wide grep for mutex/semaphore/withLock/p-limit returned **zero** concurrency primitives. Of 14 registered tools, 6 are unconditionally page-exclusive, 4 are impit-first with a page fallback, 4 never touch the page.
- **Shared page state was clobbered across callers.** `computeASI` raised the page-**global** `setDefaultTimeout` to `timeoutMs + 30_000` and reset it to a hardcoded `30_000` at three separate return points, with no save/restore and no `try/finally` — so one client's finishing compute reset a *concurrent* 180s compute's deadline to 30s, aborting it mid-`evaluate`.
- **Reinit could close the page under load** — two distinct windows, not one: `reinit()` nulls `this.page` for the whole of `init()` (a tool arriving then hit the misleading *"Client not initialized"* on a healthy daemon), and `shutdown()`'s `context.close()` rejects an already-suspended `page.evaluate` with *"Target page, context or browser has been closed"*. The `.reinit` sentinel watcher and the active-profile watcher call `client.reinit()` **directly** ([`launcher.ts:473/497/503`](packages/mcp-server/src/daemon/launcher.ts#L473)) and never pass through `getClient()`, so a barrier in the server would have been bypassed by every login and profile switch. Latent bonus: a tool re-entering `init()` mid-reinit could fire a **second `launchPersistentContext` on one browser-data** — the issue #8 deadlock class, live on main.
- **History loss is REAL but CROSS-process — the in-process race does not exist.** `history-store.js` contains zero `async`/`await`, so appends are atomic under Node's single-threaded loop; a control of 50 maximally-concurrent in-process appends lost **nothing**. The spec's "in-process index serialization" would have fixed nothing. Two *real processes* appending 25 entries each produced 50 `.md` files but only **27-31 index rows — 19-23 silently lost, 5/5 runs**, via two independent bugs: a lost-update RMW (both read the same base index; last write wins) and a **fixed `${path}.tmp`** shared by every writer (A renames it away, B's `renameSync` throws `ENOENT` on its own vanished temp; `recordToolRun` swallows it). It never self-heals — the index stays *valid* JSON, merely short, so the rebuild-on-corruption path never fires and entries are invisible until a manual rebuild.

#### Added

- **`page-scheduler.ts` — a FIFO + barrier for the one shared page** ([`page-scheduler.ts`](packages/mcp-server/src/page-scheduler.ts)). `runExclusive` serializes page work (a concurrency limit above 1 is meaningless — the page is a single tab); `runBarrier` quiesces it for reinit. It lives **on the client, not the daemon server**, because that object identity is the only seam the watcher paths and the HTTP path share — which also hands the stdio entrypoint the same protection for free. Barriers are mutually exclusive with each other, closing the double-reinit interleave (login sentinel + profile-switch watcher) that could put two `launchPersistentContext` calls on one browser-data. Reentrant via `AsyncLocalStorage`, so a nested acquire can never deadlock the daemon. The queue is capped (`daemon_queue_full`) rather than unbounded.
- **Ambient tool context.** `client.search()` backs four different tools, so the client cannot label its own work; `runWithAudit` — the single chokepoint that already computes tool name and clientId — now names the async chain, and the scheduler reads it at dequeue. Dashboards show `perplexity_research` and the owning client, not an internal method name.
- **`history-lock.js` — a synchronous, per-profile, cross-process index lock** ([`history-lock.js`](packages/mcp-server/src/history-lock.js)). Synchronous on purpose: every caller (`recordToolRun`, the dashboard's pin/tag/delete, the CLI) is sync, and going async would ripple through all of them. Reclaims a lock whose owner died (pid probe + staleness), and on acquire-timeout **proceeds unlocked with a stderr warning rather than throwing** — `recordToolRun` swallows throws, so throwing would recreate the exact silent loss being fixed.

#### Fixed

- **Concurrent page tools no longer race.** `search`, `computeASI`, `retrieveThread`, `exportThread`, `listCloudThreads`, `getCloudThread` are now one-line scheduler wrappers over private `…Unsafe` implementations. Impit fast paths are untouched: every one is gated on `if (!this.page)`, so a live daemon never pays queue cost for them, and the tool-layer impit attempts run before `getClient()` entirely.
- **`computeASI` can no longer leak its raised timeout.** A throw between the raise and a reset used to leave a ~210s default on the shared page for whatever ran next; the scheduler wrapper now restores `DEFAULT_PAGE_TIMEOUT_MS` on every exit path exactly once. The three deliberate mid-method resets are kept (one precedes `downloadASIFiles`) but now use the named constant.
- **Reinit is coordinated with in-flight work.** `reinit()` runs as a barrier: it waits out the **active** op only — deliberately *not* the whole queue, or a busy daemon would never reinit — then rebinds, then resumes. Fail-open by construction: a reinit that throws (Cloudflare block, missing browser) releases the barrier instead of freezing every client forever. `getClient()` additionally awaits `client.whenReady?.()` after its cached init promise, because `clientInitPromise` is a one-shot latch that resolves instantly *during* a reinit; a tool arriving mid-reinit now waits for the fresh page instead of getting *"Client not initialized"*. Optional-chained so existing test doubles keep working.
- **History entries are no longer lost across processes.** The lock spans read→modify→write (locking the write alone would fix nothing) for `append`/`update`/`deleteEntry`/`rebuildIndex`, and temp files are staged per-writer as `${path}.${pid}.${uuid}.tmp`. `rebuildIndex` is reachable from inside `loadIndexedEntries`/`resolveRecord` — i.e. from within a held section — so the lock is reentrant by depth, which is safe precisely because the module is synchronous.

#### Verification

- New: [`test/page-scheduler.test.js`](packages/mcp-server/test/page-scheduler.test.js) (15) — serialization vs. interleave, strict FIFO, slot released on throw, nested-acquire pass-through, `daemon_queue_full` cap, barrier waits for the active op, barrier does *not* wait for queued work, barrier blocks new ops, barriers mutually exclusive (the #8 double-launch guard), barrier fails open, `whenReady`, busy/queue reporting, ambient tool labelling, and a throwing busy listener not breaking the pipeline.
- New: [`test/history-store.concurrency.test.js`](packages/mcp-server/test/history-store.concurrency.test.js) (2) — spawns **two real processes** (an in-process test provably cannot reproduce this) appending 25 entries each and asserts 50 `.md` + 50 index rows + no leaked temps or lock. **Negative-tested:** with `PERPLEXITY_TEST_DISABLE_HISTORY_LOCK=1` it fails at **40 of 50** rows, so the test demonstrably catches the lost update.
- mcp-server suite: **698 passed / 74 files**, typecheck clean. **Manual VSIX smoke still required** for Phase A sign-off — the Chromium serialization and reinit-under-load timing cannot be exercised headlessly.

### Shared-daemon multi-client hardening — Phases B–D (contracts, fan-out, UX)

#### Added

- **`DaemonBusyState` / `DaemonBusyActive` + a `daemon:busy` ExtensionMessage** ([`messages.ts`](packages/shared/src/messages.ts)). Deliberately kept **out of** `DaemonStatusState`: that payload is rebuilt only by the heavy `postDaemonState` refresh (6+ postMessages, a 50-entry audit read, a tunnel-perf compute), so busy folded into it would be stale between refreshes and could never meet the ~1s multi-window bar. **One producer, two carriers:** the daemon's scheduler is the authority; SSE `daemon:busy` is the live transport, and the same snapshot rides `getHealth()` purely for join-time hydration — `/daemon/events` greets every new subscriber with `daemon:ready` + health, so a dashboard opened mid-research shows busy immediately instead of "Ready" until the next transition. Also mirrored into `DaemonHealthStatus` ([`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts)), a hand-maintained duplicate applied via an unchecked cast — TypeScript would not have caught the omission, the field would just have silently never arrived.
- **Busy/queue strip in the daemon card** ([`DaemonStatus.tsx`](packages/webview/src/components/DaemonStatus.tsx)). Names the humanized tool ("Research running"), the owning client, and queue depth. Passed as a **prop** on the pure `DaemonStatusView` rather than read from the store inside it — every existing test constructs that view with literal props. `null` (pre-hydrate) renders nothing rather than claiming "Ready"; idle says *"Ready — shared with your other windows & IDEs"*. Its own `aria-live="polite"` region, deliberately not nested in the bearer-reveal one below (which ticks every second during a reveal and would make busy announcements chatter). Height reserved via `min-height` so a strip appearing from another window's work never shifts the metric grid, and queue depth is a chip rather than a 4th `DaemonMetric` — the grid is 3-up and a 4th card would reflow onto a second row.

#### Changed

- **Profile switching now confirms that it is global** ([`DashboardProvider.ts`](packages/extension/src/webview/DashboardProvider.ts)). A switch rebinds the one shared daemon, silently changing the account under every other window and every attached IDE — and it had **no confirmation at all** on either side. Now a modal (mirroring the existing `profile:delete` precedent) states the blast radius. Enforced **host-side** on purpose: it also covers `ExpiredBanner`, the second entry point that sends `profile:switch`, and cannot be bypassed by a stale webview bundle. The switcher menu additionally carries an inline scope hint so the consequence is not a surprise at click time.
- **Daemon card copy** now says the shared daemon queues on the same browser session, so work started anywhere shows up in every window.

#### Fixed

- **Cancelling a profile switch no longer spins the dashboard for 60s** ([`App.tsx`](packages/webview/src/App.tsx)). `send()` optimistically starts the "Reconnecting…" spinner at send time; adding a host-side confirm meant a cancel left it spinning until the 60s safety timeout. `action:result` now stops it. Both senders mint their own `crypto.randomUUID()` and `profile:switch` is not in `ACTION_TYPES`, so the id carries no type prefix — the pending ids are tracked explicitly rather than matched by prefix.
- **One dead SSE socket no longer starves every later subscriber** ([`server.ts`](packages/mcp-server/src/daemon/server.ts)). `publishEvent` wrote unguarded inside its fan-out loop, so a throw aborted the loop mid-way. Harmless while events were rare; busy fires per tool start/finish per client, and multi-window fan-out is precisely the case that exposes it. Now each write is guarded and dead clients are dropped.

#### Verification

- New: [`tests/store.daemon-busy.test.ts`](packages/webview/tests/store.daemon-busy.test.ts) (5) — hydrate, null-vs-real-idle, host-authoritative overwrite, and non-interference with the `daemonStatus` slice. New: [`tests/DaemonStatus.busy.test.tsx`](packages/webview/tests/DaemonStatus.busy.test.tsx) (10) — `humanizeTool`, the four strip states (pre-hydrate / idle / busy / queued>0), client attribution, the live region, no secret-shaped content in the strip, and back-compat for callers passing no busy prop.
- **Invariants re-verified, not assumed:** still exactly the two pre-existing `launchPersistentContext` calls (issue #8 — and barrier mutual-exclusion now *prevents* two overlapping reinits from racing them); Doctor still self-skips a live daemon's profile; `/daemon/events` + `/daemon/health` remain absent from `TUNNEL_ALLOWLIST`, so busy is loopback-only (H11); the webview still receives `bearerAvailable` only; zero `console.log` in any new/changed daemon module (stdout stays MCP JSON-RPC); `PERPLEXITY_HEADLESS_ONLY`/`PERPLEXITY_NO_DAEMON` still stripped from the daemon spawn env; reinit callbacks still promise-wrapped; port change still re-fires MCP definitions.
- `npm run build` + `npm test`: **1290 passed / 6 skipped / 147 files** (the 6 skips are the packaging suites' by-design gate — they self-skip until `prepare:package-deps` stages `dist/node_modules`; verified 7 passed once staged).
- Phase D: multi-client smoke checklist added to [docs/smoke-tests.md](docs/smoke-tests.md) — **still required before tagging**, since real Chromium under concurrent load is exactly what the unit tests cannot reach.
...
- **Windsurf → Devin Desktop rebrand, with legacy support** ([`constants.ts`](packages/shared/src/constants.ts), [`auto-config/index.ts`](packages/extension/src/auto-config/index.ts)). Cognition rebranded the Windsurf editor to **Devin Desktop** on 2026-06-02 (in-place OTA rename) and moved workspace rules from `.windsurf/rules/` to `.devin/rules/`, keeping `.windsurf/rules/` as a read fallback. The dashboard now labels the target **"Devin Desktop (Windsurf)"** (and "Devin Desktop Next (Windsurf Next)") and auto-config **dual-writes** rules to both `.devin/rules/perplexity-mcp.md` (new primary) and `.windsurf/rules/perplexity-mcp.md` (legacy — for pre-rebrand Windsurf and the vendor fallback); detection recognizes either, and removal cleans both. The `windsurf`/`windsurfNext` target **keys** are unchanged (existing `autoConfigureWindsurf` / `mcpTransportByIde` settings keep working), and the MCP config path stays `.codeium/windsurf/mcp_config.json` (unchanged per the vendor docs). New `IdeMeta.legacyRulesPaths` field drives the dual-path behavior. Sources: [Devin Desktop FAQ](https://docs.devin.ai/desktop/devin-desktop-faq), [Cascade Memories / rules docs](https://docs.devin.ai/desktop/cascade/memories).

### Fixed

- **Session-truth sweep — the dashboard and `perplexity_models` no longer lie about auth state** ([#13](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/13)). Six coupled fixes, shipped together because several only work in combination:
  - **`perplexity_models` hardcoded `authenticated: true`** for any populated cache ([`tools.ts`](packages/mcp-server/src/tools.ts)) — anonymous accounts have a minimal models config too, so a populated cache is *not* an auth signal; anonymous users were labeled "Authenticated". The `AccountInfo` cache now carries optional `authenticated`/`userId` written by all four producers ([`session-metadata.js`](packages/mcp-server/src/session-metadata.js) for the browser runners, [`refresh.ts`](packages/mcp-server/src/refresh.ts), [`impit-login-runner.js`](packages/mcp-server/src/impit-login-runner.js)); readers default missing fields to `false` so pre-upgrade caches degrade to Anonymous, never fake a session. A single `deriveTierLabel` in [`config.ts`](packages/mcp-server/src/config.ts) replaces three divergent tier ladders.
  - **Speed Boost (impit) login wrote a broken cache**: 4 of its 5 hand-rolled REST paths didn't exist upstream (a phantom `configs` prefix) and it dumped raw keyed responses instead of the `AccountInfo` shape every reader expects — real Pro accounts were downgraded to "Authenticated" in the dashboard. It now uses `buildRuntimeEndpoints` + `deriveAccountFlags`/`deriveTier` — the same single source as everything else — and writes the correct shape. Its local tier ladder (which read an unverified `subscription_tier` field no other code references) is gone.
  - **Soft logout left you "logged in"**: [`logout.js`](packages/mcp-server/src/logout.js) now removes `models-cache.json` and `daemon-status.json` (browser-data is deliberately untouched — daemon single-owner, issue #8), and the extension's `hasStoredLogin()` ([`session.ts`](packages/extension/src/auth/session.ts)) checks `meta.lastLogin` instead of `vault.enc` existence — the vault file survives every soft logout by design (email/userId are kept), so the old file-existence check reported "logged in" forever. Note for pre-existing installs: a profile logged in by a version that never recorded `lastLogin` will need one re-login for the extension to see it.
  - **`not-logged-in` was a catch-all that misdirected users**: a vault that unsealed fine whose *live session probe* failed (expired cookie, CF block, network) rendered "run login to connect" — a login that couldn't help. New `auth-check-failed` reason in the shared `DaemonAuthStatus` contract, mapped by `computeDaemonAuthReason` ([`client.ts`](packages/mcp-server/src/client.ts)) from whether stored cookies were actually loaded, with its own honest badge text in the dashboard.
  - **A vault seal failure crashed login runners as a generic "crash"** *after* the user completed the login. All three runners now emit a typed `vault_seal_failed` (exit 6), and [`vault.js`](packages/mcp-server/src/vault.js) finally logs *why* keytar failed to load — #11's wrong-platform binary produced literally no trace anywhere.
  - **`perplexity_reason`/`perplexity_research` error taxonomy**: the "requires an authenticated Pro account" message fired only on the *authentication* check (misleading anonymous users), while authenticated Free users passed the gate silently and failed upstream. Both tools now use compute's two-step taxonomy — auth first, then a Pro/Max/Enterprise check that **fails open** when account flags never loaded, so missing data can't block a legitimate user.
  - **`perplexity://account/status` existed but never registered** — `registerResources()` always supported a provider and both transports called it one-arg. Wired via a new curated `buildAccountSnapshot()` (tier, auth, flags, rate limits — not the raw cache) on stdio and the HTTP daemon.

- **Daemon port lifecycle — VS Code no longer stays pinned to a dead port** ([#14](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/14)):
  - **`Perplexity.daemonPort` was dead code**: defined, read, nudged into by the extension's own "Pin a port now?" prompt — and consumed by nothing. [`runtime.ts`](packages/extension/src/daemon/runtime.ts) now validates (1024–65535) and passes it to `ensureDaemon`, so pinning actually pins.
  - **Definitions re-fire when the observed port moves**: every daemon-ensure funnels through `ensureBundledDaemon`, which now tracks the observed port and notifies `onBundledDaemonPortChange`; the extension re-fires `serverDefinitionsChanged` so VS Code re-resolves instead of hammering the dead port with `ECONNREFUSED`. No loop: the re-resolve lands on the same port and the notifier only fires on change.
  - **Wedged daemons are reclaimed**: `isStale()` is pid-liveness only, so a live pid whose port never answered health (hung between lock-acquire and port-sync — lock pinned at port 0 — or a dead socket under a live process) read as `running: true` forever; the spawn gate never opened and every `ensureDaemon` timed out with no recovery short of hand-deleting `daemon.lock`. [`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts) now runs a second pass: SIGTERM→SIGKILL the wedged pid (never its own), release the lock, retry once.

- **A failed background reinit hard-killed the whole daemon → `ECONNREFUSED` in Copilot** ([`reinit-watcher.js`](packages/mcp-server/src/reinit-watcher.js), [`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts)) — reported on [#10](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/10). `watchReinit` ran its callback as `try { callback(); } catch {}`, but [`launcher.ts`](packages/mcp-server/src/daemon/launcher.ts) hands it `async () => { await client.reinit(); }`. An async function returns a promise **without throwing**, so the synchronous catch never fired and the promise was discarded — a rejecting `reinit()` (Cloudflare block, profile lock, no browser installed) became an **unhandled rejection**, and Node ≥15's default `--unhandled-rejections=throw` **hard-exited the daemon**. A healthy, listening HTTP server died because a background refresh failed. VS Code kept the `http://127.0.0.1:<port>/mcp` it had already cached, so every Copilot call returned `connect ECONNREFUSED`. The asymmetry gave it away: the sibling `watchActiveProfile` callback 11 lines below already had an internal try/catch, with a comment explaining exactly why. Now guarded at both layers — the watcher runs callbacks through a promise so no future async caller can re-introduce the crash, and the `reinit` callers log to `daemon.log` so the failure is visible instead of silent. **This also corrects two diagnoses in the field:** #12's claim that "the extension watchdog respawns it" is wrong (there is **no** watchdog — respawn is lazy, only when something calls `ensureBundledDaemon()`), and #12 cannot itself cause `ECONNREFUSED` — `client.init()` is **lazy** (invoked on the first tool call), so the HTTP listener binds long before any Cloudflare interaction; a CF block yields tool-level errors on a *reachable* port. Likewise #11 does **not** kill the daemon: `vault.js` catches the `dlopen` failure. The reports chain instead: #11 leaves a Windows user Anonymous → they retry login → login touches `.reinit` → this bug kills the daemon → the stale definition pins VS Code to a dead port.

- **Windows/macOS: VSIX shipped a Linux `keytar.node`, so every login stayed Anonymous** ([`prepare-package-deps.mjs`](packages/extension/scripts/prepare-package-deps.mjs)) — [#11](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/11). Three facts composed into a **total auth outage** for every non-Linux Marketplace user: keytar 7.9.0 uses `prebuild-install` (not optionalDependencies-per-platform like `@ngrok/ngrok`), so `npm ci` materializes exactly **one** `build/Release/keytar.node` — the *build machine's*; `prepare-package-deps.mjs` listed keytar in the generic `rootPackages` array, which blind-`cpSync`s that binary; and [`release.yml`](.github/workflows/release.yml) packs a **universal** VSIX (no `vsce --target`) on `ubuntu-latest`. Result: Windows/macOS users got a Linux ELF, the loader rejected it (`Bad Image`, `0xC000012F` = `STATUS_INVALID_IMAGE_NOT_MZ` — the OS literally reporting "no initial MZ header"), keytar never loaded, the vault could never be sealed **or** unsealed, and every login silently fell back to Anonymous with all Pro tools failing. Now the four smoke-matrix prebuilds (`linux-x64`, `darwin-x64`, `darwin-arm64`, `win32-x64`) are fetched from keytar's GitHub Releases — they are **not** on npm, so the `npm pack` path used for ngrok does not apply — verified by **magic bytes** (`MZ` / `\x7FELF` / Mach-O), and staged side-by-side under `build/Release/<platform>-<arch>/`. Because keytar's `lib/keytar.js` hardcodes `require('../build/Release/keytar.node')` with no `process.platform` dispatch, side-by-side prebuilds are inert on their own, so packaging applies a surgical one-line loader patch (asserted against the stock line, so an upstream change fails the build loudly rather than silently re-shipping one platform's binary). The build machine's own prebuild is deleted so it can never be loaded by accident. Adds ~300 KB to a ~12.7 MB VSIX. **Note:** `vault.js`'s keytar error handling was *not* changed — it already try/catches correctly, which is exactly why users saw the clean `vault locked: no keychain, no env var, no TTY` message; the "Bad Image" dialog is an OS loader hard-error raised inside `process.dlopen()` that JS cannot intercept. The bug was only ever observable in CI-built artifacts — a VSIX packed locally on Windows ships a *Windows* keytar and cannot reproduce it.

- **Cloudflare re-challenges the headless search context; page tools failed in ~6.5s** ([`client.ts`](packages/mcp-server/src/client.ts)) — [#12](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/12). Every page-touching tool (`perplexity_ask` / `search` / `research`) began failing with `page.evaluate: Target page, context or browser has been closed`. **[Superseded before release — see the "visible browser window" entry above: the default is headless again and headed-offscreen is opt-in via `PERPLEXITY_PERSISTENT_HEADED=1`.]** Phase 2's persistent context briefly launched **headed-offscreen wherever a display exists** (`OFFSCREEN_POSITION_ARG` keeps the window invisible), which is what the reporter verified restores service. Deliberately **gated, not the unconditional flip** the issue proposed: headless Linux is a first-class target and has no X/Wayland, where a headed launch throws `Missing X server or $DISPLAY` — that message does **not** match `LOCK_CONTENTION_RE`, so `launchWithRetry` rethrows immediately and `init()`'s catch would kill the daemon. Today that box works precisely because only Phase 1 is headed and `headedBootstrap()` swallows its own errors, so an unconditional flip would convert a soft, recoverable failure into a hard daemon-start failure. It would also silently override `PERPLEXITY_HEADLESS_ONLY=1` for stdio-in-process, airgapped, and doctor-probe users who explicitly opted out of a UI — so that flag now forces Phase 2 headless too. New `PERPLEXITY_PERSISTENT_HEADED=1|0` overrides the decision in both directions. **Root cause is not yet isolated** and the fix may be treating a symptom: the issue's "~6.5s watchdog" **does not exist** (no watchdog, no `6500` constant anywhere) — a far better fit for a *constant* ~6.5s is that [`fs-utils.js`](packages/mcp-server/src/fs-utils.js)'s `LOCK_CONTENTION_RE` literally matches `"target page, context or browser has been closed"`, so `launchWithRetry` burns 4 attempts + 1.4s backoff ≈ 6.5s. [`probe.js`](packages/mcp-server/src/checks/probe.js) already attributes that exact string to **#8 singleton-lock contention** (the reporter had a daemon pile-up), and their Chrome auto-updated to 150 one day before onset against pinned patchright `^1.59.4`. Their repro also used a *fresh* profile, which only proves headless cannot **solve** a challenge — something `client.ts` already documents — not that it cannot **reuse** Phase 1's `cf_clearance`, which is what the design actually relies on. Disambiguating questions posted on the issue.

- **Daemon reported "anonymous session" forever once it stopped running** ([`session.ts`](packages/extension/src/auth/session.ts)) — surfaced by [#10](https://github.com/Automations-Project/VSCode-Perplexity-MCP/issues/10) follow-ups ("cant get it to change out of this status"). `shutdown()` sets `authenticated:false` and writes `daemon-status.json` on its way out, and a crashed daemon simply leaves its last write behind — so a dead daemon stranded `{authenticated:false, reason:"not-logged-in"}` on disk, which the dashboard rendered permanently as **"Daemon sees an anonymous session — run login to connect."**, telling users to run a login that could never clear it. The status file carried `pid` all along but was read with no liveness check; `getAccountSnapshot()` now ignores a status whose writing process is gone (pre-`pid` daemons are still trusted, so older daemons degrade safely).

- **`engines.vscode` floor was below the MCP API the extension depends on** ([`package.json`](packages/extension/package.json)). Declared `^1.100.0`, but `lm.registerMcpServerDefinitionProvider` landed in ~1.101 and VS Code's MCP support is documented as 1.102+. The extension therefore installed cleanly on 1.100/1.101, silently skipped MCP registration, and left users with no server and no tools picker — with only a generic warning. Floor raised to `^1.102.0`.

### Verification

- **Session truth**: [`test/session-truth.test.js`](packages/mcp-server/test/session-truth.test.js) (tier ladder, all four `computeDaemonAuthReason` branches, `buildAccountSnapshot` incl. the pre-upgrade-cache-degrades-to-Anonymous case, and a source-level regression pinning the impit runner to shared endpoints), [`logout.test.js`](packages/mcp-server/test/logout.test.js) (cache/status removal), [`has-stored-login.test.ts`](packages/extension/tests/has-stored-login.test.ts) (true after login → false after soft logout while `vault.enc` still exists).
- **Port lifecycle**: [`daemon-port-change.test.ts`](packages/extension/tests/daemon-port-change.test.ts) (fires only on change, honors valid pins, refuses 0/out-of-range) and a [`launcher.test.js`](packages/mcp-server/test/daemon/launcher.test.js) wedge test against a **real live process** holding the lock at port 0 — the exact shape that used to dead-end every ensure.

- **Reinit daemon-kill**: [`test/reinit-watcher.test.js`](packages/mcp-server/test/reinit-watcher.test.js) spawns a **real subprocess** (vitest installs its own rejection handler, so an in-process test cannot observe the exit that defines the bug) and asserts it survives a rejecting async callback. Negative-tested: against the pre-fix `try { callback(); } catch {}` the subprocess exits 1 — i.e. the test reproduces the daemon death — and passes on the fix. Plus a sync-throw case.
- **#11**: [`tests/keytar-cross-platform-packaging.test.ts`](packages/extension/tests/keytar-cross-platform-packaging.test.ts) asserts each supported platform's `keytar.node` by **magic bytes** (not by path or `package.json` os/cpu — the ngrok sibling test checks only manifest fields, which is exactly the check that *cannot* catch a wrong-platform binary at a right-looking path), that the build machine's prebuild is gone, that the loader is patched, and that the current runner's binary resolves. Negative-tested: planting the Linux ELF at `win32-x64/` fails the suite with the intended message. End-to-end on Windows 11: the patched loader resolves `win32-x64`, loads the real PE, and round-trips a secret through the actual Windows Credential Manager.
- **#12**: [`test/client-launch-options.test.js`](packages/mcp-server/test/client-launch-options.test.js) covers the new `resolvePhase2Headless` decision (headed on macOS/Windows; headless on Linux with no display; headed on Linux with `DISPLAY`/`WAYLAND_DISPLAY`; `PERPLEXITY_HEADLESS_ONLY` honored; `PERPLEXITY_PERSISTENT_HEADED` overriding both ways). The pre-existing tests only covered `buildLaunchOptions` itself and would not have caught the Linux regression.
- **Stale daemon-status**: [`tests/daemon-status-liveness.test.ts`](packages/extension/tests/daemon-status-liveness.test.ts) covers live/dead/legacy/absent/malformed.
- **Still needs the manual VSIX smoke on Windows 11 + macOS 14+ + Ubuntu 22+ before tagging** ([docs/smoke-tests.md](docs/smoke-tests.md)) — and for #11 the smoke **must** use the CI-built artifact, since a locally packed VSIX ships the local platform's keytar and cannot reproduce the bug.

- New unit tests: [`test/prompts-config.test.js`](packages/mcp-server/test/prompts-config.test.js) + [`test/prompts.test.js`](packages/mcp-server/test/prompts.test.js) (mcp-server — `{arg}` substitution, merge/override flags, upsert/delete/reset, read-write round-trip with a temp `PERPLEXITY_CONFIG_DIR`, dynamic registration against a real `McpServer`) and [`tests/prompts-handler.test.ts`](packages/extension/tests/prompts-handler.test.ts) + [`tests/auto-config.devin-rules.test.ts`](packages/extension/tests/auto-config.devin-rules.test.ts) (extension — prompts save/delete/reset validation; Devin/Windsurf dual-write + legacy detect + remove). Full build + typecheck clean across all four packages; **1225 tests pass**. **Still needs the manual VSIX smoke** before a release is tagged — the live dashboard render and the rules-file/`/slash-command`-in-a-client round-trips can't be exercised headlessly.
...
- **`client.reinit({ passphrase })`** ([`client.ts`](packages/mcp-server/src/client.ts)) sets the env and `vault.__resetKeyCache()` **only when a passphrase is supplied**, so a passphrase change isn't blocked by the previous profile's pinned `_unsealMaterialCache`/`_keyCache`. The reset is deliberately scoped: the OS-keychain master key is shared across all profiles (`KEYTAR_ACCOUNT = "vault-master-key"`), so keychain-based unseals (typical macOS/Windows) stay valid across a profile switch and are **never** reset — clearing the cache would re-import keytar and re-trigger macOS Keychain permission prompts (issue #6 bug 3). Linux/passphrase users are unaffected (they supply the passphrase and get the reset they need).
