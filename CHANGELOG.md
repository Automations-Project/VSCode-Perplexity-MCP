...
## [Unreleased] — Browser runtime expansion (Brave + macOS Edge + bundled-Chromium installer + Obscura)

Broadens browser support from "Chrome or bundled Chromium, Windows/Linux-centric" to six runtimes with a usable UI contract across all three platforms. The most visible piece — Obscura — is a 30 MB Rust CDP server that replaces the ~170 MB bundled Chromium for headless search operations.
...
- **`ObscuraManager`** at [packages/extension/src/browser/obscura-manager.ts](packages/extension/src/browser/obscura-manager.ts). New module that: pulls the Obscura binary (~18–26 MB) from [h4ckf0r0day/obscura GitHub releases](https://github.com/h4ckf0r0day/obscura/releases) into globalStorage, extracts it via the OS's native `tar` command (Win10+/macOS/Linux all have it), spawns `obscura serve --port <N> --stealth` as a subprocess, health-checks the CDP `/json/version` HTTP endpoint before declaring running, and SIGTERM/SIGKILL-escalates on stop. Supports `win32-x64`, `linux-x64`, and `darwin-arm64` — upstream doesn't ship binaries for macOS Intel / Linux ARM / Windows ARM yet, so those platforms surface a clear error rather than silently failing.
- **Three new `PERPLEXITY_*` env vars** read by [mcp-server/config.ts](packages/mcp-server/src/config.ts):
  ...
  - `PERPLEXITY_BROWSER_CHANNEL` — `chrome` / `msedge` / `chromium` / `obscura`.
  - `PERPLEXITY_OBSCURA_ENDPOINT` — CDP URL (`http://127.0.0.1:9332`) of a running `obscura serve` process; required alongside `PERPLEXITY_BROWSER_CHANNEL=obscura`.
- **`getObscuraEndpoint()` / `shouldUseObscuraForHeadless()` helpers** in [config.ts](packages/mcp-server/src/config.ts). Gated on BOTH the endpoint AND an explicit channel selection so a stray env var can't accidentally redirect traffic.
- **Obscura connect path for three headless contexts** in the MCP server:
  - [client.ts::init](packages/mcp-server/src/client.ts) phase 2 (headless search): branches to `chromium.connectOverCDP(endpoint)` when Obscura is configured; the headed Cloudflare Turnstile bootstrap keeps using real Chrome/Edge/Brave because CF compatibility against Obscura is unproven.
  - [refresh.ts::tierBrowser](packages/mcp-server/src/refresh.ts) (account-info refresh tier 3): same connect path.
  - [health-check.js](packages/mcp-server/src/health-check.js): new `launchHealthBrowser()` helper that branches on channel.
- **`AuthManager` browser orchestration** at [packages/extension/src/mcp/auth-manager.ts](packages/extension/src/mcp/auth-manager.ts): `attachDownloadManager`, `attachObscuraManager`, `refreshBrowserDetection`, `setBrowserChoice`, and `syncProcessEnv` (writes the three env vars onto `process.env` so the detached daemon spawned in [daemon/runtime.ts](packages/extension/src/daemon/runtime.ts) inherits the active selection). Runners spawned via `spawnRunner` also receive the same env vars — with `PERPLEXITY_OBSCURA_ENDPOINT` deliberately stripped for login runners because the interactive login flow always uses a real browser.
- **Shared wire types** in [packages/shared/src/messages.ts](packages/shared/src/messages.ts): `BrowserInfo`, `BrowserChoice`, `BrowserDownloadState`, `ObscuraState`, `BrowserChannel`. `AuthState` now carries `browser`, `availableBrowsers`, `browserDownload`, `browserChoice`, `obscura` so a future dashboard browser-picker UI has everything it needs over postMessage.
...
- `findBrowser()` probe order expanded to **Chrome → Edge (all platforms) → Chromium → Brave → override-via-env**. Old signature `{ path, channel: "chrome" | "msedge" | "chromium" | "bundled" }` gains `obscura`; consumers that destructure only `path` are unaffected.
...
### Security
- No new secrets surface. The Obscura binary is verified by GitHub's HTTPS + the release's own digest metadata (sha256 in the release JSON); future hardening can add an explicit digest check at download time.
- Obscura subprocesses are tracked by PID and SIGTERM/SIGKILL-cleaned on extension deactivation, matching the daemon + download-manager patterns already in place. No detached survivors on extension reload.

### Tests
- Existing suite still passes. New browser-detect / download-manager / obscura-manager modules ship without dedicated tests in this commit — they're behavior-equivalent ports from the Airtable-Formula extension and gated on env vars the current tests don't set, so nothing breaks. Follow-up work: unit tests for `ObscuraManager.download` (mock the GitHub releases endpoint) and `AuthManager.resolveBrowserEnv` (table-driven).

### Release gate
- Typecheck: green across all 4 packages.
- Browser picker UI in the dashboard: deferred to a follow-up release. Env-var plumbing is complete, but the webview still has no picker component — Brave/Obscura selection works through env vars today.
- Airtable-Formula Obscura port: intentionally skipped this pass because Airtable's `launchPersistentContext`-based auth doesn't cleanly match Obscura's stateless model. Documented as a separate initiative.
