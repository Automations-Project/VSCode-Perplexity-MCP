...
- **VS Code can stay pinned to a dead daemon port.** The daemon binds an OS-assigned **ephemeral** port on every start (`options.port ?? 0`), that port is baked into the `McpHttpServerDefinition`, and `serverDefinitionsChanged` only fires on user-initiated auth/profile actions — **never** on daemon death or respawn. So if the daemon ever dies, VS Code keeps calling the old port forever, and even a healthy respawn lands on a port it never learns about. The reinit fix above removes the main *cause* of daemon death, but the stale-definition problem itself remains. Compounding it: **`Perplexity.daemonPort` is dead code** — it is defined and read, but never reaches the spawn path, so the extension's own nudge ("this config will break when the daemon restarts on a new port. Pin `Perplexity.daemonPort`?", which writes `49217`) is a **no-op**. Fix is either to honor the setting or to re-fire the definitions-changed event when the observed port moves.
